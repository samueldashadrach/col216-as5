...
int main() {
    ...
    unordered_map<string , int8_t> lmap;    // Maps labels to integers
    unordered_map<int8_t , int> PCmap;     // Maps the integers to PC
    ...
    while (file){
        ...
        if(regex_match(str,m,re_label))
        {
            ...
            if (lmap.find(string(m[1])) == lmap.end()){

                lmap[string(m[1])] = LabelNumber;
                PCmap[LabelNumber] = PC;
                LabelNumber++;
            }
            else{
                PCmap[lmap.at(string(m[1]))] = PC;
            }
            ...
            if (lmap.find(string(m[3])) == lmap.end()){                
                lmap[string(m[3])] = LabelNumber;
                Memory[PC + 3] = LabelNumber;
                LabelNumber++;
            }
            else
                Memory[PC + 3] = lmap.at(string(m[3]));
            ...
            if (lmap.find(string(m[1])) == lmap.end()){                
                ...
                Memory[PC + 1] = lmap.at(string(m[1]));
    ...
    while( (Memory[EC]!=11 && !Break)  || !InsQueue.isEmpty() )
    {
        //cout << "Loop here" << endl;
    	...
    	// Actual execution
    	if(Memory[EC] == 0 && (DepReg[Memory[EC+2] ] == 0) && (DepReg[Memory[EC+3] ] == 0 )) // add
    	{
    		reg[Memory[EC+1]] = reg[Memory[EC+2]] + reg[Memory[EC+3]];
            ...
            printReg(Memory[EC + 1]);
            cout <<", ";
            printReg(Memory[EC + 2]);
            cout <<", ";
            printReg(Memory[EC + 3]);

            cout << "\t executed" << endl;
            cout << "Register " ;
            printReg(Memory[EC+1]);
            cout << " updated to " << reg[Memory[EC+1]] << endl << endl; 

            if (DepReg[Memory[EC + 1]] != 0)
                DepReg[Memory[EC+1]]++;
    	
            EC+=4;
        }
    	else if(Memory[EC] == 1 && (DepReg[Memory[EC+2] ] == 0) && (DepReg[Memory[EC+3] ] == 0 )) // sub
    	{
    		reg[Memory[EC+1]] = reg[Memory[EC+2]] - reg[Memory[EC+3]];
    	...
    	else if(Memory[EC] == 2 && (DepReg[Memory[EC+2] ] == 0) && (DepReg[Memory[EC+3] ] == 0 )) // mul
    	{
    		reg[Memory[EC+1]] = reg[Memory[EC+2]] * reg[Memory[EC+3]];
    	...
    	else if(Memory[EC] == 3 && (DepReg[Memory[EC+2] ] == 0)) // muli
    	{
    		reg[Memory[EC+1]] = reg[Memory[EC+2]] * Memory[EC+3];
            ...
            cout << (int)Memory[EC + 3] << "\t executed" << endl;
            cout << "Register " ;
            printReg(Memory[EC+1]);
            cout << " updated to " << reg[Memory[EC+1]] << endl<< endl; 
    	...
    	else if(Memory[EC] == 4 && (DepReg[Memory[EC+1] ] == 0) && (DepReg[Memory[EC+2] ] == 0 )) // beq
    	{
            ...
            cout << ", label ID: " << (int)Memory[EC+3] << "\t executing" << endl;
 

    		if(reg[Memory[EC+1]] == reg[Memory[EC+2]])
            {
                try
                {
                    cout << "Jumping to Label ID: " << (int)Memory[EC+3] << endl<< endl;
                    EC = PCmap.at(Memory[EC+3]);
                }
                catch(exception& e)
                {
    	...
    	else if(Memory[EC] == 5 && (DepReg[Memory[EC+1] ] == 0) && (DepReg[Memory[EC+2] ] == 0 )) // bne
    	{
    	...
    	else if(Memory[EC] == 6 && (DepReg[Memory[EC+2] ] == 0) && (DepReg[Memory[EC+3] ] == 0 )) // slt
    	{
    		if(reg[Memory[EC+2]] < reg[Memory[EC+3]])
    			reg[Memory[EC+1]] = 1;
    		else
    			reg[Memory[EC+1]] = 0;
    	...
    	else if(Memory[EC] == 7) // j
    	{


            cout << "Cycle " << stat_tot << " -> j label ID: "<< (int)Memory[EC+1] << "\t executing" << endl;
            try
            {
                cout << "Jumping to Label ID: " << (int)Memory[EC+1] << endl<< endl;
                EC = PCmap.at(Memory[EC+1]);    
            }
    		catch(exception& e)
            {
                cout<<"Error. Label not found";
    	...
    	else if(Memory[EC] == 8 && DepReg[Memory[EC+2]] == 0 ) // lw
    	{

            Instruction Temp;

            Temp.Code = 1;
            Temp.Reg = Memory[EC+1];
            Temp.baseRegVal = reg[Memory[EC+2]];
            Temp.Offset = to16bit(Memory[EC+3], Memory[EC+4]);
    	...
    	else if(Memory[EC] == 9 && DepReg[Memory[EC+2]] == 0 && DepReg[Memory[EC+1]] == 0) // sw
    	{

            Instruction Temp;

            Temp.Code =2;
            Temp.Reg = reg [Memory[EC+1]];
            Temp.baseRegVal = reg [Memory[EC+2]];
            Temp.Offset = to16bit(Memory[EC+3], Memory[EC+4]);
    	...
    	else if(Memory[EC] == 10 && DepReg[Memory[EC+2]] == 0) // addi
    	{
    		reg[Memory[EC+1]] = reg[Memory[EC+2]] + to16bit(Memory[EC+3], Memory[EC+4]);
            ...
            cout << to16bit(Memory[EC+3], Memory[EC+4]);
        ...
        while(!InsQueue.isEmpty() && Temp.Code != 2 && DepReg[Temp.Reg] != 1) {
            //cout << "Reducing" << endl;
            DepReg[Temp.Reg]--;            
            InsQueue.DeleteMin(); 
        ...
        if( !InsQueue.isEmpty() && Temp.Code == 1){     // LW code
            ...
            if(Temp.Offset + Temp.baseRegVal <= RBIndex*1024 + 1023 && Temp.Offset + Temp.baseRegVal >= RBIndex*1024){
                ...
                if (Temp.StartClock + ColAD == Clock){
                    ...
                    reg[Temp.Reg] = to32bit(Memory[location], Memory[location + 1], Memory [location +2], Memory[location + 3]);
                    
                    cout <<endl<<"Cycle " << Clock + 1 - ColAD << " - " << Clock << " -> Reading from Memory address "<< Temp.Offset + Temp.baseRegVal << endl << endl;
                    cout <<"Register " ;
                    printReg(Temp.Reg);
                    cout << " updated to " << reg[Temp.Reg] << endl << endl;
